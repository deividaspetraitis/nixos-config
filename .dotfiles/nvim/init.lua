...
-- Auto-format files before saving
vim.api.nvim_create_autocmd("BufWritePre", {
	...
	callback = function()
		...
		if #clients > 0 then
			...
			-- there could be multiple workspace folders
			-- keep it simple for now and just check the first one
			local ws = workspaces[1]
			if ws then
				local noformat = vim.fs.find(".noformat", {
					path = ws,
					upward = false, -- only check inside workspace root
					type = "file",
				})[1]
