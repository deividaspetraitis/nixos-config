{
  ...
  outputs = { self, nixpkgs, nixpkgs-stable, sops-nix, deploy-rs, nixos-hardware, home-manager, ... } @ inputs:
    let
      # Target system
      system = "x86_64-linux";
      ...
      mkDeployNode = import ./modules/mkDeployNode.nix {
      ...
      nixosConfigurations = {
        ...
        "m715q-0" = nixpkgs.lib.nixosSystem {
          ...
          modules = [
            ...
            ({ config, pkgs, ... }: { nixpkgs.overlays = [ overlay-stable ]; })
            sops-nix.nixosModules.sops
            ./hosts/m715q-0/configuration.nix
          ];
        };

        "m715q-1" = nixpkgs.lib.nixosSystem {
          inherit system;
          modules = [
            # Overlays-module makes "pkgs.unstable" available in configuration.nix
            ({ config, pkgs, ... }: { nixpkgs.overlays = [ overlay-stable ]; })
            ./hosts/m715q-1/configuration.nix
            sops-nix.nixosModules.sops
          ];
        };
